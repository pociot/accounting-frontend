...
export const getUser = (username: string, password: string): ThunkAction<void, null, null, Action> => dispatch => {
  dispatch(getUserBegin({}));
  getData(username, password).then(
    (response) => {
  ...
  ).catch(error => dispatch(getUserFail({ error: error.response.data.error_description })));
};